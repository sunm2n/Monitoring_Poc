...
| 4. 앱은 백엔드를 몰라도 된다 | ✅ | ✅ 동일 |
| 리소스 사용량 | ⚠️ 1.29 GiB | ✅ **219 MiB** |
| 임의 필드 검색·집계 | ✅ 전 필드 | ⚠️ 라벨 외에는 전체 스캔 |

**한 줄 요약**: Loki 는 압도적으로 싸고 가볍지만, **이 PoC 의 핵심인 회사별 권한 분리를 할 수 없다.**
...
## 3. 리소스 — 여기서는 Loki 가 압도한다

같은 로그를 받고 있는 상태에서 측정했다.

| 컨테이너 | 메모리 | CPU |
|---|---|---|
| `poc-opensearch` | **1.079 GiB** | 1.01% |
| `poc-dashboards` | 214.7 MiB | 0.38% |
| **OpenSearch 합계** | **≈ 1.29 GiB** | |
| `poc-loki` | **113.1 MiB** | 1.19% |
| `poc-grafana` | 105.5 MiB | 0.29% |
| **Loki 합계** | **≈ 219 MiB** | |
| `poc-fluent-bit` | 4.7 MiB | 0.04% |

**약 6배 차이다.** JVM 힙을 512m 로 낮춰 잡은 상태에서도 이렇다.

> 🎯 **온프렘 납품에서 이게 결정적일 수 있다.**
> 고객사가 4GB 램 VM 하나를 주는 상황이라면 OpenSearch + Dashboards 는 사실상 그것만으로 꽉 찬다.
> Loki 는 같은 자리에 여유롭게 들어간다.
