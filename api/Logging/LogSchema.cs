...
public static class LogSchema
{
    ...
    ///   Fluent Bit 은 벌크 청크 단위로 재시도한다. 그런데 응답을 읽지 못하면
    ///   서버가 이미 처리했는지 알 수 없어서 청크를 통째로 다시 보낸다.
    ///   문서 ID 가 자동 생성이면 그 재전송이 곧 중복이다.
    ...
    ///
    ///   ※ 당시 재전송을 유발한 http_do=-1 은 "연결 수준 실패" 로 진단했었는데 틀렸다.
    ///     실제로는 Fluent Bit 의 응답 버퍼(Buffer_Size, 기본 512KB)가 벌크 응답보다 작아
    ///     응답을 읽지 못한 것이었다. 자세한 경위는 docs/volume-test.md 참조.
    ///     원인이 무엇이든 이 필드의 필요성은 그대로다 — 재전송은 언제든 일어난다.
    /// </summary>
