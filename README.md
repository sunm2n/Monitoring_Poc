...
| 회사별 격리 | ✅ | ❌ 인증 계층 자체가 없다 |
| 필드 숨김 (FLS) | ✅ | ❌ 개념 없음 |
| 메모리 | 수집 처리량에 비례해 힙 필요 | 수집량에 비례하지 않음 |
...
| 구성 | 500만 건 유실 |
|---|---|
| 기본 설정 | 1,732,312건 (**34.65%**) |
| + Fluent Bit `Buffer_Size False` | 805,507건 (16.1%) |
| + Docker `fluentd-buffer-limit 256MB` | **1건 (0.00002%)** |

원인은 **두 개의 설정 기본값**이었고, 둘 다 하드웨어와 무관하다.

- **Fluent Bit `Buffer_Size` 512KB** — OpenSearch 벌크 *응답*을 담을 버퍼가 부족해
  응답을 읽지 못하고 실패로 처리한다. `http_do=-1` 은 네트워크 오류가 아니었다.
  색인은 정상 완료(`index_failed=0`)됐는데 수집기만 실패로 알고 재시도했다.
- **Docker `fluentd-buffer-limit` 1MB** — `fluentd-async` 링버퍼가 넘치면
  수집기에 닿기도 전에 **조용히** 버린다. 앱에도 수집기에도 기록이 남지 않는다.

남은 1건은 버스트의 첫 줄(`seq=1`)로, async 연결이 서기 전에 쓰여 버려진 것이다.

> 📌 **가장 값비싼 교훈**: 첫 진단이 틀렸다. "메모리 버퍼 + 낮은 Retry_Limit" 이라 보고
> 고쳤더니 유실이 20.47% → 30.25% 로 **악화**됐다. Fluent Bit 계측(`HTTP_Server On`)을
> 켜자 30분 만에 진짜 지점이 나왔다. **계측 없는 진단은 추측이다.**

문서당 **224 bytes** 로 수렴하고, **DLS 오버헤드는 사실상 0** 이었다 (436만 건 기준 47ms → 50ms).

> ⚠️ **위 유실률은 개발 노트북 기준이라 그대로 가져갈 수 없다.** 측정 시점에 Docker VM 이
> 스왑을 거의 다 쓴 상태였고, 단일 노드 · 1 샤드 구성이다. 어떤 숫자가 하드웨어와 무관하고
> 어떤 숫자를 버려야 하는지는 [docs/volume-test.md](docs/volume-test.md) 상단에 정리해 뒀다.
